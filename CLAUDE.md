...
pnpm workspace。`packages/web`（React Router v8 + shadcn/ui + Conform）と `packages/api`（Hono on Workers + Drizzle + D1）。web は `/api/*` を Service Binding で api に中継する同一オリジン構成。認証は Google OIDC のみ（Phase 1）。

Phase 1（MVP）。画面 1〜3（インテーク `/`、プロジェクト `/projects`、STAR `/memos`）とタグ管理 `/tags` は実装済み、画面 4（レジュメ `/resume`）は未着手。マルチテナントを意識しつつ過剰実装はしない。

## 作業ガイドライン

1. 不明な点は質問する。推測でコードを書かない
2. 設計判断が必要な変更は事前にユーザーに確認する
3. 既存のコーディング規約・ディレクトリ構成を尊重する
4. 大きな変更は小さなコミット単位に分割する
5. 新しいライブラリは提案にとどめ、勝手に追加しない
6. TDD で進める（Red → Green → Refactor）。ドメインロジック・分岐・境界にテストを集中させ、型で保証される部分や単純な通過コードは追わない。詳細は `docs/07-testing.md`
