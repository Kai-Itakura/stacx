...
- `:provider` はコールバック URL のパス（`/api/auth/callback/:provider`）から取得できるため、provider 用の Cookie は持たない（一時 Cookie は上記 2 つのみ）
...
`Secure` は本番・ローカルとも常時付与する。`http://localhost` はブラウザ仕様上 secure context として扱われ、Secure Cookie が受理・送信されるため、ローカルでも `Secure` を外す必要はない（「ローカルは Secure 不可」は現在のブラウザでは当てはまらない）。

環境差は **`__Host-` プレフィックス（と Cookie 名）だけ**で、`APP_BASE_URL` が `https://` で始まるか（=本番）で切り替える。`http://`（ローカル）ではプレフィックスなしの `stacx_session` を使う。`__Host-` はローカルでも概ね通るが、localhost を LAN IP や独自ホスト名で開くと secure context 扱いされず拒否されうるため、名前を分けて安全側に倒す。
...
- D1 で `sessions` を引き、有効性確認（Session → User → 最新 Identity の 1 JOIN、純 SELECT）
- ユーザー情報 `{ id, email, name, pictureUrl }` を Hono の `c.var.user` に注入。無効・期限切れは 401
- Phase 1 の保護ルート例は `GET /api/me`（`c.var.user` をそのまま返す。フロントの「現在のユーザー」取得用）
...
| ケース | 挙動 |
|---|---|
| `state` 不一致・欠落 | 302 リダイレクト → `{APP_BASE_URL}/login?error=state_mismatch`（ブラウザ遷移中のため JSON でなくリダイレクト） |
| `code` 交換・検証失敗 | 302 リダイレクト → `{APP_BASE_URL}/login?error=oauth_failed` |
